...
const TimeSelect: React.FC = () => {
  const { schedules, selectedDates, setSelectedDates, addSchedule, addTimeToSchedule, removeTimeFromSchedule } =
  ...
  return (
    <div>
      <div className="relative">
        <button onClick={toggleDatePicker} className="bg-[#4D80FF] text-white p-2 rounded-md hover:bg-blue-700">
          일정 추가
        </button>
        {isDayPickerOpen && (
      ...
      <div className="flex flex-col my-4">
        {selectedDates.map((date, index) => (
          <div key={index} className="flex items-center gap-2 my-2">
            ...
            <button onClick={() => handleAddTime(date)} className="bg-green-500 text-white p-1 rounded-md">
              +
            </button>
