...
const RegisterEditPage = () => {
  ...
  return (
    <div className="mx-auto max-w-4xl p-4">
      ...
      <div className="border border-[#FCFCFF] bg-[#FCFCFF] px-1 py-4 shadow-md sm:px-2">
        <div className="p-4 md:p-6 lg:p-8">
          <h1 className="text-md mt-1 md:text-lg lg:text-xl">클래스 기본정보 입력란</h1>
          <hr className="my-4 border-[#4D43B8]" />
          <p className="mt-1 text-sm text-[#7E7E7E]">* 표시는 필수 입력 항목입니다</p>
          ...
          <h1 className="text-md mt-16 md:text-lg lg:text-xl">클래스 세부요소 입력란</h1>
          <hr className="my-4 border-[#4D43B8]" />
          <Address
            isEditMode={true}
          ...
          <SelectTime
            isEditMode={true}
            initialData={{ schedules: dateData.map((dateItem) => ({ date: dateItem.day, times: dateItem.times })) }}
            class_Id={classId as string}
          />

          <h1 className="text-md mt-16 md:text-lg lg:text-xl">클래스 금액</h1>
          <hr className="my-4 border-[#4D43B8]" />
          <Price isEditMode={true} initialData={{ price: classData.price }} />

          <h1 className="text-md mt-16 md:text-lg lg:text-xl">이미지 업로드</h1>
          <hr className="my-4 border-[#4D43B8]" />
