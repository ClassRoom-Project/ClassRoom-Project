      ...
      {!isLoading && !isError ? (
        <>
          <div className="w-full min-h-100vh-header-default  box-border bg-light-purple  flex justify-center mobile:justify-center mobile:py-2 items-center flex-col text-gray-700">
            <div className=" mobile:w-11/12 lg:w-1/2 xl:min-w-1/2 sm:w-11/12 md:w-4/5  md:min-w-[500px] lg:min-w-[600px] 2xl:w-[48%] lg:bg-slate-400 xl:bg-slate-700 2xl:bg-orange-400   justify-center min-h-[600px] bg-white shadow rounded-md items-center flex flex-col px-12 mobile:px-6 pt-12 pb-8">
              <FiCheckCircle color="#38c557" className="mb-6 mobile:mb-4" size={70} />
              ...
              <div className="flex flex-col items-center w-full justify-center">
                <div className="flex flex-col bg-base-200 mobile:text-sm sm:w-4/5 md:w-4/5 lg:bg-black lg:w-8/12 rounded-lg py-4 items-center mb-5 gap-2">
                  <p className="text-xl font-bold text-center mx-4 mobile:text-base sm:text-lg">
                ...
                <div className="flex flex-col gap-4 font-bold w-10/12 sm:w-2/3 md:w-2/3 truncate xl:w-[55%] lg:w-[55%] lg:min-w-[290px] 2xl:max-w-[500px]">
                  {reserveInfoLabels.map(({ icon, title, description }) => (
                    <div key={title} className="flex gap-2 w-full">
                      <div className="flex flex-row gap-2 justify-center items-center">
                        ...
                        <p className="mobile:hidden sm:flex flex-shrink-0 mr-2">{title}</p>
                      </div>
                      <p
                        className={`text-left text-gray-600 font-normal   mobile:text-sm md:text-base sm:font-normal truncate`}
                      >
                        {description}
                      </p>
                    </div>
