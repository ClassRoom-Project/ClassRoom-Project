  ...
  return (
    <div className="responsiveHeight">
      <div className="h-screen min-w-100vw m-0 lg:bg-400 flex flex-col">
        {!isLoading && !isError ? (
          <>
            <div className="w-full h-full  bg-light-purple flex justify-center  items-center flex-col text-gray-700">
                  ...
                  <div className="flex flex-col gap-4 font-bold max-w-10/12 w-2/3 sm:w-8/12 md:w-2/3 truncate lg:w-[45%] lg:min-w-[290px] 2xl:max-w-[500px]">
                    {reserveInfoLabels.map(({ icon, title, description }) => (
                      <div key={title} className="flex gap-2 w-full">
                        ...
                        <p
                          className={`text-left text-gray-600 font-normal text-sm sm:font-normal md:text-base truncate`}
                        >
                          {description}
                        </p>
                      </div>
