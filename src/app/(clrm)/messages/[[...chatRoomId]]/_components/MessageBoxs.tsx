...
export default function MessageBoxs({ toClassId, title, chatId, otherId, studentName, mainImage }: MessagesBoxsType) {
  ...
  const { readChatRoomMessages, isLoading, setCursorDate } = useReadChatRoomMessages(chatId, loginUserId!);
  ...
  useEffect(() => {
    ...
    const handleScroll = (e) => {
      if (element && readChatRoomMessages && readChatRoomMessages.length > 0) {
        const { scrollTop } = element;

        //스크롤이 맨위에 도달
        if (scrollTop === 0) {
          console.log('Reached top');
          const newCursorDate = readChatRoomMessages[readChatRoomMessages.length - 1].created_at;
          setCursorDate(newCursorDate);
        }
      }
    };
  ...
  }, [readChatRoomMessages, setCursorDate]);
    ...
    <div className="h-screen overflow-y-auto " ref={scrollRef}>
      ...
      <div className="flex flex-col items-center justify-center px-3">
        ...
        <div className="flex w-full flex-col items-center justify-center px-3">
          {readChatRoomMessages?.toReversed().map((message, index) => (
            <div
              key={index}
